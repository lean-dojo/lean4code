...
function packageTask(platform, arch, sourceFolderName, destinationFolderName, opts) {
	...
	return () => {
		...
		const productJsonStream = gulp.src(['product.json'], { base: '.' })
			.pipe(json({ commit, date: readISODate('out-build'), checksums, version, serverDownloadUrlTemplate: 'https://github.com/VSCodium/vscodium/releases/download/1.100.33809/vscodium-reh-${os}-${arch}-1.100.33809.tar.gz' }))
			.pipe(es.through(function (file) {
